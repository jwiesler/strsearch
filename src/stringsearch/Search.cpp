...
namespace stringsearch {
	std::span<const Index> Search::find(const std::wstring_view pattern) const {
		const auto lower = lowerBound(pattern);
		const auto upper = upperBound(pattern);
		return MakeSubSpan(sa_, lower, upper);
	}

	std::span<const Index>::const_iterator Search::lowerBound(const std::wstring_view pattern) const {
	...
	std::span<const Index>::const_iterator Search::upperBound(const std::wstring_view pattern) const {
